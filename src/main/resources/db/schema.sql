...
-- 2. 公共字段库表
CREATE TABLE IF NOT EXISTS UCMS_PUBLIC_FIELD_LIBRARY (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  field_name VARCHAR(50) NOT NULL COMMENT '字段名称（姓名、民族、性别等）',
  field_name_en VARCHAR(50) NOT NULL COMMENT '英文字段名称（name、nation、gender等）',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  is_deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除, 1-已删除'
) COMMENT='公共字段库表';

-- 2.1 公共字段库表索引
CREATE INDEX idx_field_name ON UCMS_PUBLIC_FIELD_LIBRARY(field_name);
CREATE INDEX idx_is_deleted ON UCMS_PUBLIC_FIELD_LIBRARY(is_deleted);
...
CREATE TABLE IF NOT EXISTS UCMS_CERTIFICATE_TYPE_FIELD (
  ...
  field_name VARCHAR(50) NOT NULL COMMENT '字段名称（name、cert_number等）',
  field_display_name VARCHAR(50) NOT NULL COMMENT '字段显示名称（姓名、证件号码等）',
  is_required TINYINT NOT NULL DEFAULT 0 COMMENT '是否必填：0-否, 1-是',
  field_type VARCHAR(20) NOT NULL COMMENT '字段类型（text/number/date等）',
  field_length INT COMMENT '字段长度',
  sort_order INT NOT NULL DEFAULT 0 COMMENT '排序',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  is_deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除, 1-已删除',
  FOREIGN KEY (cert_type_code) REFERENCES UCMS_CERTIFICATE_TYPE(cert_type_code)
) COMMENT='证件类型字段配置表';
...
CREATE TABLE IF NOT EXISTS UCMS_VALIDATION_RULE_CONFIG (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  business_type VARCHAR(100) COMMENT '业务类型（如档案补正等）',
...
CREATE TABLE IF NOT EXISTS UCMS_VALIDATION_RULE_ITEM (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  is_deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标记：0-未删除, 1-已删除',
  FOREIGN KEY (rule_id) REFERENCES UCMS_VALIDATION_RULE_CONFIG(id)
) COMMENT='校验规则明细表';
